...
static int pCus_poweroff(ms_cus_sensor *handle, u32 idx)
{
    ...
    sensor_if->SetCSI_Clk(idx, CUS_CSI_CLK_DISABLE);
    sensor_if->SetCSI_Lane(idx, handle->interface_attr.attr_mipi.mipi_lane_num, 0);
    CamOsMsSleep(1);
...
static int __init sigmastar_sc2231_init_driver(void)
{
    int nCamID=0;
...
static void __exit sigmastar_sc2231_exit_driver(void)
{
    pr_info("sensordrv exit");
}

subsys_initcall(sigmastar_sc2231_init_driver);
module_exit(sigmastar_sc2231_exit_driver);

MODULE_DESCRIPTION("Sensor_sc2231");
