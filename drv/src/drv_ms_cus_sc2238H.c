...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(SC2238H);
...
#define SENSOR_MAXGAIN      (16 * 8* 196875 / 100000)                  // max sensor again, a-gain * conversion-gain*d-gain
...
const I2C_ARRAY Sensor_init_table[] =     //cleaned_0x01_FT_SC2238H_DVP_27Minput_10bit_74.25MPclk_1920x1080_30fps.ini
{
{0x0103,0x01},
{0x0100,0x00},
{0x301f,0x01},
...
#endif
static int sc2238Hpoweron(ms_cus_sensor *handle, u32 idx)
{
...
static int sc2238Hpoweroff(ms_cus_sensor *handle, u32 idx)
{
    ISensorIfAPI *sensor_if = &handle->sensor_if_api;
...
static int sc2238HGetSensorID(ms_cus_sensor *handle, u32 *id)
{
    int i,n;
...
static int sc2238HSetPatternMode(ms_cus_sensor *handle,u32 mode)
{

    return SUCCESS;
}

static int sc2238HInit(ms_cus_sensor *handle)
{
    int i,cnt=0;
    sc2238H_params *params = (sc2238H_params *)handle->private_data;
    //ISensorIfAPI *sensor_if = &(handle->sensor_if_api);
...
static int sc2238HGetVideoResNum( ms_cus_sensor *handle, u32 *ulres_num)
{
    *ulres_num = handle->video_res_supported.num_res;
    return SUCCESS;
}
static int sc2238HGetVideoRes(ms_cus_sensor *handle, u32 res_idx, cus_camsensor_res **res)
{
...
static int sc2238HGetCurVideoRes(ms_cus_sensor *handle, u32 *cur_idx, cus_camsensor_res **res)
{
    u32 num_res = handle->video_res_supported.num_res;
...
static int sc2238HSetVideoRes(ms_cus_sensor *handle, u32 res_idx)
{
    u32 num_res = handle->video_res_supported.num_res;
    ...
    switch (res_idx) {
        case 0:
            ...
            handle->pCus_sensor_init = sc2238HInit;
            break;
        default:
            break;
    }
...
static int sc2238HGetOrien(ms_cus_sensor *handle, CUS_CAMSENSOR_ORIT *orit)
{
...
static int sc2238HSetOrien(ms_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit)
{
...
static int sc2238HGetFPS(ms_cus_sensor *handle)
{
  sc2238H_params *params = (sc2238H_params *)handle->private_data;
  return  params->expo.fps;
}
static int sc2238HSetFPS(ms_cus_sensor *handle, u32 fps)
{
    u32 vts=0;
    sc2238H_params *params = (sc2238H_params *)handle->private_data;
    SENSOR_DMSG("\n\n ****************  [%s], fps=%d  **************** \n", __FUNCTION__, fps);
    ...
    {
        params->expo.fps = fps;
        params->expo.vts=  ((vts_30fps*30*10000+(142857>>1))/142857)-1;
    }
    else if(fps>=3 && fps <= 30) {
        params->expo.fps = fps;
...
static int sc2238HAEStatusNotify(ms_cus_sensor *handle, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    sc2238H_params *params = (sc2238H_params *)handle->private_data;
    switch(status)
    {
...
static int sc2238HGetAEUSecs(ms_cus_sensor *handle, u32 *us)
{
    int rc=0;
    u32 lines = 0;
    sc2238H_params *params = (sc2238H_params *)handle->private_data;
...
static int sc2238HSetAEUSecs(ms_cus_sensor *handle, u32 us)
{
    int i;
    u32 lines = 0, vts = 0;
    sc2238H_params *params = (sc2238H_params *)handle->private_data;
...
static int sc2238HGetAEGain(ms_cus_sensor *handle, u32* gain)
{
    int rc = 0;
...
static int sc2238HSetAEGain(ms_cus_sensor *handle, u32 gain)
{
     sc2238H_params *params = (sc2238H_params *)handle->private_data;

    u8 i=0 , Dgain = 1, Coarse_gain = 1;
    u8 Dgain_reg = 0, Coarse_gain_reg = 0, Fine_gain_reg= 0x20;
...
static int sc2238HGetAEMinMaxUSecs(ms_cus_sensor *handle, u32 *min, u32 *max)
{
  *min = 30;
...
static int sc2238HGetAEMinMaxGain(ms_cus_sensor *handle, u32 *min, u32 *max)
{
  *min = 1024;
...
static int sc2238HGetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int sc2238HReleaseHandle(ms_cus_sensor *handle)
{
    return SUCCESS;
}

int sc2238HInitHandle(ms_cus_sensor* drv_handle)
{
    ms_cus_sensor *handle = drv_handle;
    sc2238H_params *params = 0;
    if (!handle)
    {
    ...
    params = (sc2238H_params *)handle->private_data;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->model_id,"sc2238H_PARL");
    ...
    handle->pCus_sensor_release     = sc2238HReleaseHandle;
    handle->pCus_sensor_init        = sc2238HInit;
    handle->pCus_sensor_poweron     = sc2238Hpoweron ;
    handle->pCus_sensor_poweroff    = sc2238Hpoweroff;

    // Normal
    handle->pCus_sensor_GetSensorID     = sc2238HGetSensorID;

    handle->pCus_sensor_GetVideoResNum = sc2238HGetVideoResNum;
    handle->pCus_sensor_GetVideoRes       = sc2238HGetVideoRes;
    handle->pCus_sensor_GetCurVideoRes  = sc2238HGetCurVideoRes;
    handle->pCus_sensor_SetVideoRes       = sc2238HSetVideoRes;

    handle->pCus_sensor_GetOrien        = sc2238HGetOrien;
    handle->pCus_sensor_SetOrien        = sc2238HSetOrien;
    handle->pCus_sensor_GetFPS          = sc2238HGetFPS;
    handle->pCus_sensor_SetFPS          = sc2238HSetFPS;
    handle->pCus_sensor_SetPatternMode  = sc2238HSetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    handle->pCus_sensor_AEStatusNotify  = sc2238HAEStatusNotify;
    handle->pCus_sensor_GetAEUSecs      = sc2238HGetAEUSecs;
    handle->pCus_sensor_SetAEUSecs      = sc2238HSetAEUSecs;
    handle->pCus_sensor_GetAEGain       = sc2238HGetAEGain;
    handle->pCus_sensor_SetAEGain       = sc2238HSetAEGain;
    handle->pCus_sensor_SetAEGain_cal   = sc2238HSetAEGain;
    handle->pCus_sensor_GetShutterInfo  = sc2238HGetShutterInfo;

    handle->pCus_sensor_GetAEMinMaxGain = sc2238HGetAEMinMaxGain;
    handle->pCus_sensor_GetAEMinMaxUSecs= sc2238HGetAEMinMaxUSecs;
...
SENSOR_DRV_ENTRY_IMPL_END_EX( SC2238H,
                            sc2238HInitHandle,
                            NULL,
                            NULL,
                            sc2238H_params
