...
static int pCus_SetFPS(ms_cus_sensor *handle, u32 fps)
{
  ...
  //  pr_info("[%s]  max_min_fps : %d ,%d\n\n", __FUNCTION__,max_fps,min_fps);
	...
	{
		params->expo.fps = fps;
		tmp_vts = ((vts_30fps * max_fps * 10000)/142857);
        params->expo.vts=  (u32)tmp_vts;
	}
...
static int pCus_SetFPS_HDR_DOL_SEF(ms_cus_sensor *handle, u32 fps)
{
	...
	{
		params->expo.fps = fps;
        tmp_vts = (vts_30fps_HDR_DOL * max_fps * 10000)/142857;
		params->expo.vts = (u32)tmp_vts;
        vts_reg[0].data = (params->expo.vts >> 8) & 0x00ff;
        vts_reg[1].data = (params->expo.vts >> 0) & 0x00ff;
        params->reg_dirty = true;
...
//  pr_info("[%s]  max_min_fps : %d ,%d\n\n", __FUNCTION__,max_fps,min_fps);
	...
	{
		params->expo.fps = fps;
        tmp_vts = (vts_30fps_HDR_DOL * max_fps * 1000 * 10000)/142857;
		params->expo.vts = (u32)tmp_vts;
        vts_reg[0].data = (params->expo.vts >> 8) & 0x00ff;
        vts_reg[1].data = (params->expo.vts >> 0) & 0x00ff;
        params->reg_dirty = true;
